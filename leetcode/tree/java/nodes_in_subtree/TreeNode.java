...
class TreeNode {
  ...
  List<Node> findSubTree(Node root, int n) {
    if (root.val == n) {
    ...
    for (Node nd: child) {
      if (findSubTree(nd, n).size() >0) {
        return findSubTree(nd,n);
      }
    }
  ...
  int count(List<Node> sub, char c, String s) {
    if (sub.size()==0) {
      return 0;
    }
    ...
    for (Node n: sub) {
      if (c == s.charAt(n.val-1)) {
        ans = ans+1;
      }
      
      ans = ans + count(n.children, c, s);
  ...
  int[] countOfNodes(Node root, ArrayList<Query> queries, String s) {
    ...
    for (int i=0; i<queries.size(); i++) {
      ...
      //find substree
      List<Node> sub = findSubTree(root, u);
      
      ans[i] = 1 + count(sub, c, s);
